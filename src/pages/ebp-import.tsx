...
const CurrentlyUpdatingAlert: FC = () => {
  ...
  if (typeof currentProduct === "undefined") {
    ...
    message = `(${stockUpdateState.processedCount + 1}/${
      stockUpdateState.totalCount
    }) Mise à jour des stocks pour [${currentProduct.ebpId}] "${
      currentProduct.name
    }"`;
  }
  ...
  return (
    <>
      ...
      {stockUpdateState.errorCount > 0 && (
        <Alert variant="danger">
          {stockUpdateState.errorCount} erreur(s) durant la mise à jour des
          stocks
        </Alert>
      )}
    </>
...
const EbpImport: FC = () => {
  ...
  const makeAlert = () => {
    ...
    return <Alert variant={getAlertClass()}>{getAlertContent()}</Alert>;
  };
  ...
  return (
    <Container>
      ...
      <p>
        Cette page permet d'importer les données exportées du logiciel EBP, afin
        de mettre à jour les stocks des produits dans GraphCMS.
      </p>
        ...
        <Card>
          <Card.Header>
            ...
            <AccordionCollapse eventKey="0">
              <Card.Body>
                <p>La structure attendue de l'export EBP est la suivante:</p>
                <ol>
                ...
                <p>
                  Note: les colonnes annotées d'une astérisque (*) doivent être
      ...
      <Form className="mb-4" onSubmit={handleSubmit}>
        <Form.Group>
          <Form.File
            ...
            onChange={(event: ChangeEvent<HTMLInputElement>) =>
              setCurrentFile(event.target.files && event.target.files[0])
            }
            custom
          />
        </Form.Group>
        ...
        <Button type="submit" disabled={currentFile === null}>
          Envoyer
        </Button>
      </Form>
      {makeAlert()}
